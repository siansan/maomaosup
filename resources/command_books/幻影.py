            ...
            if abs(d_y)>settings.adjust_tolerance:
                if direction == 'down':
                    ...
                    if abs(d_y) >500:
                        ...
                        key_up('left')
                        key_up('down')
                        press(Key.tp,down_time=0.001)
                        time.sleep(0.05)
                        press("up",1,down_time=0.5)
                        time.sleep(0.05)
                        press(Key.tp,down_time=0.01)
                        time.sleep(0.2 + 0.2 * random())
                    else:
                        up_jump_press(Key.jump, 1)
                        time.sleep(0.1)
                        up_jump_press(Key.jump, 1, num_presses)
            ...
            while config.enabled and counter > 0:
                ...
                if abs(d_y) > settings.adjust_tolerance:
                    if d_y < 0:
                        ...
                        if abs(self.target[1] - config.player_pos[1])>20:
                            key_down('down')
                            press(Key.jump, 1, down_time=0.3)
                            time.sleep(0.1)
                            key_up('down')
    ...
    def main(self):
        ...
        if Key.key11!="f12":
            if self.ir==0 or (now-self.ir)>60:
                ...
                time.sleep(0.05)
                key_up("down")
class switch_e(Command):
    def __init__(self,value):
        super().__init__(locals())
        self.n=int(value)
    def main(self):
        runearrow.switch_13FFF0CF0(int(self.n))

class switch_shootobj(Command):
    def main(self):
        runearrow.switch_shootobj()
...
class Arrow_Blaster(Command):
    def __init__(self,value):
        super().__init__(locals())
        self.n=float(value)
    def main(self):
        key_down(Key.Arrow_Blaster)
        time.sleep(0.1)
        key_down('up')
        time.sleep(self.n)
        key_up('up')
        press('a')
        key_up(Key.Arrow_Blaster)