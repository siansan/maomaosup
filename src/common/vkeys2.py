...
def press(key, n=1, down_time=0.05, up_time=0.05,nd=None):
    ...
    if nd==None:
        for _ in range(n):
            key_down(key)
            time.sleep(down_time * (0.3 + 0.4 * random()))
            key_up(key)
            time.sleep(up_time * (0.3 + 0.4 * random()))
    else:
        for _ in range(n):
            key_down(key)
            time.sleep(0.01)
...
def click(position, button='left'):
    ...
    if button not in ['left', 'right']:
        ...
        print(position[0], position[1]+100)
...
def mouse_move():
    ...
    p['x']=pos[0]
    p['y']=pos[1]
    win32api.SetCursorPos((p['x']+randint(-100,100),p['y']+randint(-100,100)))
