    ...
    if global_var.map_id==runearrow.map_():
        if global_var.buff_stop==0:
            ...
            if abs(d_y)>settings.adjust_tolerance:
                if direction == 'down':
                    if abs(d_y) >100:
                        press(Key.jump, 2, num_presses,up_time=0.00001)
                        # time.sleep(0.6 + 0.1 * random())
                        if runearrow.action_()>=12:
                            while True:
                                key_down(direction)
                                time.sleep(0.5)
...
class key9(Command):
    ...
    def main(self):
        if Key.key9!="f12":
            press(Key.key9, 3)
class jump_onikiri(Command):

    def main(self):
        if Key.jump_onikiri!="f12":
            press(Key.jump,1,down_time=0.0000001,up_time=0.00001)
            time.sleep(0.1)
            press(Key.jump_onikiri, 1)
